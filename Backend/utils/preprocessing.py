...
class XGBoostPreprocessor(BasePreprocessor):
    ...
    def add_xgb_specific_features(self, df):
        ...
        try:
            ...
            if len(df_result) >= 14:
                lowest_low = df_result['Low'].rolling(window=14, min_periods=1).min() if 'Low' in df_result.columns else df_result['Close'].rolling(window=14, min_periods=1).min()
                highest_high = df_result['High'].rolling(window=14, min_periods=1).max() if 'High' in df_result.columns else df_result['Close'].rolling(window=14, min_periods=1).max()
                df_result['stoch_k'] = 100 * (df_result['Close'] - lowest_low) / (highest_high - lowest_low + 1e-10)
            
            # Williams %R
            if len(df_result) >= 14:
                df_result['williams_r'] = -100 * (highest_high - df_result['Close']) / (highest_high - lowest_low + 1e-10)
